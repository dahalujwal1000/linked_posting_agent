...
export async function generateWithFallback(providers: AiProvider[], prompt: string, log: (event: { provider: string; model: string; attempt: number; error?: string }) => void, timeoutMs = 20_000): Promise<GeneratedDraft> {
  let lastError: unknown;
  for (const provider of providers) for (let attempt = 0; attempt < 2; attempt += 1) {
    const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), timeoutMs);
    try { const raw = await provider.generate(prompt, controller.signal); const parsed = generatedDraftSchema.safeParse(raw); if (!parsed.success) throw new AiProviderError("Invalid structured output", attempt === 0 ? "retryable" : "permanent"); log({ provider: provider.name, model: provider.model, attempt }); return parsed.data; }
    catch (error) { lastError = error; log({ provider: provider.name, model: provider.model, attempt, error: error instanceof Error ? error.message : "Unknown provider failure" }); if (classifyProviderError(error) === "permanent") break; if (attempt === 0) await new Promise(resolve => setTimeout(resolve, 300)); }
